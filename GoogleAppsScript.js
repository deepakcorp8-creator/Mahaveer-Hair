...
function doPost(e) {
  ...
  if (action == 'addPackage') {
    ...
    sheet.appendRow([
      formatDateForSheet(data.startDate), // Store as 'DD/MM/YYYY
      data.clientName,
      data.packageName,
      data.totalCost,
      data.totalServices,
  ...
  if (action == 'editPackage') {
    ...
    try {
        ...
        sheet.getRange(rowId, 1).setValue(formatDateForSheet(data.startDate));
  ...
  if (action == 'addEntry') {
    ...
    sheet.appendRow([
      formatDateForSheet(data.date), // Store as 'DD/MM/YYYY
      data.clientName,
      data.contactNo,
      data.address,
      data.branch,
  ...
  if (action == 'updateEntryStatus') {
    ...
    try {
        ...
        return response({status: "success"});
    } catch(e) { return response({error: "Failed to update status"}); }
  }
  ...
  if (action == 'addUser') {
      ...
      if (exists) return response({status: "error", message: "User already exists"});
...
function getOptions(ss) {
    ...
    return response({clients: clients, technicians: technicians, items: items});
}
...
function getUsers(ss) {
    ...
    const users = data.map(row => ({
        username: row[0], password: row[1], role: row[2], department: row[3], permissions: row[4]
    }));
...
// FORMAT FOR SHEET STORAGE: 'DD/MM/YYYY (Text)
function formatDateForSheet(isoDate) {
  if (!isoDate || typeof isoDate !== 'string') return isoDate;
  var parts = isoDate.split('-');
  if (parts.length === 3) {
    // Prepends ' to force Google Sheet to treat as text and NOT auto-format
    return "'" + parts[2] + '/' + parts[1] + '/' + parts[0];
  }
  return isoDate;
}

// FORMAT FOR DISPLAY (No Quote)
...
// Convert DD/MM/YYYY or Date Object to YYYY-MM-DD (ISO) for App
function formatDate(date) {
  ...
  // Check for YY
  var matchShort = s.match(/^(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{2})$/);
  if (matchShort) {
     var day = ('0' + matchShort[1]).slice(-2);
     var month = ('0' + matchShort[2]).slice(-2);
     var year = '20' + matchShort[3];
     return year + '-' + month + '-' + day;
  }
...
function createInvoicePDF(data) {
  ...
  // Use clean display format for PDF
  var displayDate = formatDateForDisplay(data.date); 
  ...
  blob.setName("INV_" + data.clientName + "_" + displayDate.replace(/\//g,'-') + ".pdf");
