...
const AdminPanel: React.FC = () => {
    ...
    const handleGenerateAuditLog = async () => {
        if (!confirm("Generate full database backup? This may take a moment.")) return;
        setExporting(true);
        try {
            // 1. Fetch all data
            const [entries, appointments, packages, options, currentUsers] = await Promise.all([
                api.getEntries(true),
            ...
            const entriesData = entries.map(e => ({
                ...
                'Patch Size': e.patchSize || '-',
                'Patch Method': e.patchMethod || '-',
                Technician: e.technician,
                Amount: e.amount,
                'Received': e.amount - (e.pendingAmount || 0),
                'Pending': e.pendingAmount || 0,
                'Payment Mode': e.paymentMethod,
                Status: e.workStatus,
                Remark: e.remark
            }));
            const wsEntries = XLSX.utils.json_to_sheet(entriesData);
            XLSX.utils.book_append_sheet(wb, wsEntries, "Transactions");

            // 3. Clients Sheet
            const clientsData = (options.clients || []).map((c: any) => ({
                Name: c.name,
                Contact: c.contact,
                City: c.address || '-',
                Source: c.source || '-',
                'Total Visits': c.visitCount || 0,
            ...
            const pkgData = packages.map(p => ({
                ...
                'Total Services': p.totalServices,
                'Used (Old)': p.oldServiceCount || 0,
                Status: p.status
            }));
            const wsPkg = XLSX.utils.json_to_sheet(pkgData);
            XLSX.utils.book_append_sheet(wb, wsPkg, "Packages");

            // 6. Staff Sheet
            const staffData = currentUsers.map((u: any) => ({
                'Staff ID': u.username,
                'Role': u.role,
            ...
            XLSX.utils.book_append_sheet(wb, wsStaff, "Staff Access");

            // Save File
            const fileName = `Mahaveer_Full_DB_Backup_${new Date().toISOString().split('T')[0]}.xlsx`;
            XLSX.writeFile(wb, fileName);
            alert("Backup generated successfully!");
        } catch (e) {
            console.error(e);
            alert("Failed to generate backup. Please check console.");
        } finally {
            setExporting(false);
        }
    };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-950 p-8 rounded-[2.5rem] shadow-2xl text-white relative overflow-hidden group">
                        ...
                        <button onClick={handleGenerateAuditLog} disabled={exporting} className="w-full py-4 bg-white/10 hover:bg-white/20 rounded-2xl font-black text-sm uppercase tracking-[0.2em] transition-all border border-white/10 backdrop-blur-xl relative z-10 shadow-xl active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed">
                            {exporting ? 'GENERATING...' : 'GENERATE FULL DATABASE BACKUP'}
                        </button>
                    </div>
                </div>
            </div>
