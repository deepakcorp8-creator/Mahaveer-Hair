...
const ClientMaster: React.FC = () => {
  ...
  const loadClients = async () => {
    setLoading(true);
    try {
      // Optimization: Use cached data (false) instead of forcing refresh (true)
      const data = await api.getOptions(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowModal(false);
      setFormData({ name: '', contact: '', address: '', gender: 'Male', email: '', dob: '' });
      // Background refresh after update
      loadClients();
    } catch (e) {
      console.error(e);
                ...
                <tr>
                  <td colSpan={5} className="px-8 py-20 text-center">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p className="text-slate-400 font-black uppercase tracking-widest text-xs">Loading Clients...</p>
                    </div>
                  </td>
                </tr>
              ) : (
        ...
        {!loading && filteredClients.length > itemsPerPage && (
          <div className="p-4 border-t border-slate-100 bg-slate-50 flex items-center justify-between">
            <button
              ...
              className="px-4 py-2 rounded-lg bg-white border border-slate-200 text-slate-600 font-bold text-xs hover:bg-slate-100 disabled:opacity-50 disabled:cursor-not-allowed uppercase tracking-wider"
            >
              Previous
            </button>
