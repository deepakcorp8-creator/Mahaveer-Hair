...
import { History, Search, UserSearch, RotateCcw, FileDown, Printer, User, Wallet, Calendar, ArrowUpRight, Filter, ArrowRight, ChevronDown, CalendarCheck, ListFilter, X, RefreshCw } from 'lucide-react';
...
const ClientHistory: React.FC = () => {
    ...
    }, [entries, branchFilter, startDate, endDate, searchTerm]);

    const totalAmount = useMemo(() => filteredData.reduce((sum, e) => sum + Number(e.amount), 0), [filteredData]);
    ...
    return (
        <div className="flex flex-col space-y-8 animate-in fade-in duration-500 pb-20">
                ...
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <button onClick={() => loadData(true)} className="p-2 rounded-full hover:bg-slate-100 text-slate-500 hover:text-indigo-600 transition-colors" title="Refresh Data">
            ...
            <div className="bg-white p-5 rounded-[2rem] shadow-xl shadow-slate-200/60 border border-slate-200 space-y-5 relative overflow-hidden group">
                ...
                {/* Top Row: Core Filters (Date & Branch) */}
                <div className="grid grid-cols-1 md:grid-cols-12 gap-4 relative z-10">
                    ...
                    <div className="md:col-span-4 relative group">
                        ...
                        />
                        <label className="absolute -top-2.5 left-5 px-2 bg-white text-[10px] font-black text-slate-400 uppercase tracking-widest rounded-full border border-slate-100">From Date</label>
                    </div>
                    ...
                    <div className="md:col-span-4 relative group">
                        ...
                        <input
                            ...
                            className="w-full pl-16 pr-4 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl text-sm font-black text-slate-700 outline-none focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 transition-all cursor-pointer"
                        />
                        <label className="absolute -top-2.5 left-5 px-2 bg-white text-[10px] font-black text-slate-400 uppercase tracking-widest rounded-full border border-slate-100">To Date</label>
                    </div>
                    ...
                    <div className="md:col-span-4 relative group">
                        ...
                        <label className="absolute -top-2.5 left-5 px-2 bg-white text-[10px] font-black text-slate-400 uppercase tracking-widest rounded-full border border-slate-100">Filter Branch</label>
                    </div>
                </div>
            ...
            {totalPages > 1 && (
                <div className="flex flex-col md:flex-row justify-between items-center bg-white p-4 rounded-2xl shadow-sm border border-slate-200 gap-4 mt-6">
                    <div className="text-sm font-bold text-slate-500 order-2 md:order-1">
                        Showing {((currentPage - 1) * itemsPerPage) + 1} to {Math.min(currentPage * itemsPerPage, totalVisits)} of {totalVisits} entries
                    </div>
