...
const ClientHistory: React.FC = () => {
  ...
  return (
    <div className="flex flex-col space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      {/* 2. Advanced Filters Panel (FIXED ALIGNMENT & 3D) */}
      <div className="bg-white p-3 rounded-[2rem] shadow-xl shadow-slate-200/60 border border-slate-200 flex flex-col xl:flex-row gap-4 items-center">
           ...
           {/* Date Selectors - Now in a dedicated container */}
           <div className="w-full xl:w-auto flex-1 bg-slate-50 rounded-3xl border border-slate-200 p-2 flex flex-col md:flex-row items-center gap-2 relative">
                ...
                <div className="relative w-full group">
                    <div className="absolute top-1/2 left-4 -translate-y-1/2 w-10 h-10 rounded-2xl bg-white shadow-sm border border-slate-100 flex items-center justify-center text-slate-400 group-focus-within:text-indigo-500 group-focus-within:border-indigo-200 transition-all">
                    ...
                    <input 
                        ...
                        className="w-full pl-16 pr-4 py-4 bg-white border-2 border-transparent hover:border-slate-200 focus:border-indigo-500 rounded-2xl text-sm font-black text-slate-700 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] transition-all outline-none"
                    />
                    <label className="absolute -top-2.5 left-6 px-2 bg-white text-[10px] font-black text-slate-400 uppercase tracking-widest rounded-full border border-slate-100 shadow-sm">
                        From Date
                    </label>
                </div>

                {/* Arrow Separator */}
                <div className="text-slate-300 px-2 flex justify-center">
           ...
           {/* Filter Presets */}
            <div className="flex w-full xl:w-auto items-center gap-2 p-1.5 bg-slate-100 rounded-[1.5rem] border border-slate-200 overflow-x-auto">
                <button 
                    ...
                    className={`px-6 py-3.5 rounded-2xl text-xs font-black transition-all flex-shrink-0 uppercase tracking-wide border shadow-sm
                ...
                <button 
                    ...
                    className={`px-6 py-3.5 rounded-2xl text-xs font-black transition-all flex-shrink-0 uppercase tracking-wide border shadow-sm flex items-center justify-center whitespace-nowrap
                    ${!startDate && !endDate && searchTerm
                        ? 'bg-violet-600 text-white border-violet-700 shadow-violet-300'
                ...
                >
                 ...
                 <button 
                    ...
                    className="p-3.5 rounded-2xl border border-slate-200 bg-white text-slate-400 hover:text-red-500 hover:bg-red-50 hover:border-red-200 transition-colors shadow-sm flex-shrink-0"
                    title="Reset Filters"
                >
      ...
      <div className="sticky top-0 z-20 -mx-4 px-4 md:-mx-8 md:px-8 bg-[#F8FAFC]/90 backdrop-blur-md py-4">
            <div className="relative group shadow-xl shadow-indigo-900/5 rounded-[1.5rem]">
                <div className="absolute left-5 top-1/2 transform -translate-y-1/2 pointer-events-none p-2 bg-indigo-50 rounded-xl">
                    <Search className="w-5 h-5 text-indigo-600 transition-transform group-hover:scale-110" />
                </div>
                <input 
                    ...
                    className="w-full pl-16 pr-6 py-4 bg-white border-2 border-white focus:border-indigo-500 rounded-[1.5rem] text-base focus:ring-4 focus:ring-indigo-100 font-bold shadow-sm transition-all placeholder:font-medium placeholder:text-slate-400 text-slate-800"
                />
            </div>
      </div>

      {/* 4. Results Table - Card Style Rows */}
             ...
             <table className="w-full text-left text-sm text-slate-600">
                 ...
                 <tbody className="divide-y divide-slate-100">
                     {loading ? (
                         ...
                         <tr><td colSpan={6} className="text-center py-24 text-slate-400 font-medium">
                         ...
                         filteredData.map((entry, idx) => (
                             <tr key={idx} className="hover:bg-slate-50 transition-colors group">
                                 <td className="px-8 py-5 whitespace-nowrap">
                                     <div className="font-bold text-slate-700">{new Date(entry.date).toLocaleDateString('en-GB')}</div>
                                 ...
                                 <td className="px-8 py-5">
                                     ...
                                     <div className="flex items-center gap-2 mt-1">
                                         <span className="text-[11px] font-bold text-slate-500 bg-white shadow-sm px-2 py-0.5 rounded border border-slate-200">{entry.contactNo}</span>
                                     </div>
                                 </td>
                                 <td className="px-8 py-5">
                                 ...
                                 <td className="px-8 py-5">
                                     <div className="flex items-center gap-2">
                                         <span className="font-bold text-xs uppercase text-slate-600 bg-white border border-slate-200 px-3 py-1.5 rounded-lg shadow-sm">{entry.paymentMethod}</span>
                                     </div>
                                 </td>
                                     ...
                                     <div className="flex items-center justify-center gap-3 opacity-0 group-hover:opacity-100 transition-all transform translate-y-1 group-hover:translate-y-0">
                                         {entry.invoiceUrl && entry.invoiceUrl.startsWith('http') && (
                                             <a 
                                                ...
                                                target="_blank" 
                                                rel="noreferrer"
                                                className="p-2.5 rounded-xl bg-indigo-50 text-indigo-600 hover:bg-indigo-600 hover:text-white border border-indigo-200 transition-all shadow-sm hover:shadow-indigo-300 hover:-translate-y-0.5"
                                         ...
                                         <button
                                            ...
                                            className="p-2.5 rounded-xl bg-slate-50 text-slate-600 hover:bg-slate-800 hover:text-white border border-slate-200 transition-all shadow-sm hover:shadow-lg hover:-translate-y-0.5"
                                            title="Print Invoice"
                                         >
