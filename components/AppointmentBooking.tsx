...
const AppointmentBooking: React.FC = () => {
    ...
    const loadData = async () => {
        ...
        // Normalize dates locally to ensure DD/MM/YYYY is handled correctly as Day/Month/Year
        const normalizedAppts = apptData.map(a => {
            let cleanDate = a.date;
            if (!cleanDate) return a;
        ...
        const sorted = normalizedAppts.sort((a, b) => {
    ...
    const formatTimeDisplay = (timeStr: any) => {
        ...
        // FIX: Strip leading single quote if present (from text-forcing)
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (newAppt.clientName && newAppt.date) {
                // FIX: Prepend single quote to force Sheet to treat as TEXT
                const payload = { ...newAppt };
                if (payload.time && !payload.time.startsWith("'")) {
                    payload.time = "'" + payload.time;
                }

                await api.addAppointment(payload as Appointment);
            ...
            <div className="flex flex-col lg:flex-row gap-8">
                ...
                <div className="lg:w-2/3 order-2 space-y-6">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white p-2.5 rounded-2xl border border-slate-200 shadow-sm">
                        <div className="flex bg-slate-100 p-1.5 rounded-xl w-full lg:w-auto border border-slate-200 overflow-x-auto no-scrollbar">
                            <button
                                onClick={() => setActiveTab('SCHEDULE')}
                                className={`flex-1 lg:flex-none px-4 md:px-6 py-2.5 rounded-lg text-sm font-black transition-all duration-300 flex items-center justify-center whitespace-nowrap
