                ...
                {filteredPackages.map(pkg => {
                    ...
                    <div key={pkg.id} className="group relative bg-white rounded-3xl p-6 shadow-[0_15px_30px_-5px_rgba(0,0,0,0.08)] border border-slate-200 hover:shadow-[0_25px_50px_-12px_rgba(99,102,241,0.2)] transition-shadow duration-300 overflow-hidden isolate">
                      ...
                      <div className="relative flex justify-between items-start mb-6 z-10">
                        <div className="flex gap-4">
                          ...
                          <div>
                             ...
                             <div className="text-xs text-slate-400 mt-1 font-semibold">Start: {formatDateDDMMYYYY(pkg.startDate)}</div>
                          </div>
                        </div>
                        {/* Status Badge */}
                      ...
                      <div className="relative z-10 flex justify-between items-center pt-4 border-t border-slate-100">
                          <span className="text-xs font-bold text-slate-400">Total Value: <span className="text-slate-700">₹{pkg.totalCost}</span></span>
                                  ...
                                  {isPending ? (
                                      <>
                                        <button 
                                            ...
                                            className="p-2 bg-emerald-50 text-emerald-600 rounded-lg hover:bg-emerald-100 border border-emerald-200 transition-colors"
                                            title="Approve"
                                        >
                                      ...
                                      <button 
                                        ...
                                        className="flex items-center text-xs font-bold text-indigo-600 hover:text-indigo-800 bg-indigo-50 px-3 py-1.5 rounded-lg transition-colors border border-indigo-100"
                                      >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
              <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                  <div className="px-6 py-4 bg-slate-100 border-b border-slate-200 flex justify-between items-center">
                      <h3 className="font-bold text-slate-800">Edit Package</h3>
                      <button 
                        onClick={() => { setIsEditModalOpen(false); setEditingPackage(null); }}
                  ...
                  <form onSubmit={handleUpdatePackage} className="p-6 space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                          ...
                          <div>
                              <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Total Services</label>
                              <input 
                                  type="number" 
                                  className="w-full rounded-xl border border-slate-300 p-3 text-sm font-semibold"
                      ...
                      <button 
                          type="submit" 
                          disabled={loading}
                          className="w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-lg transition-colors flex justify-center items-center mt-2"
                      >
